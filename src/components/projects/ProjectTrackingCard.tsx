...
import { Upload, Gauge, FileText, Download, Eye, Trash2 } from 'lucide-react'
import type { DeadlineConfidence, Project, ScopeFile, TrackingScopeStatus } from '@/types'
import { Card } from '@/components/ui/Card'
import { Button } from '@/components/ui/Button'
import { useToast } from '@/components/ui/Toast'
import { addScopeFile, removeScopeFile, updateProjectTracking } from '@/services/projectsService'
...
export function ProjectTrackingCard({ project, onProjectChange }: ProjectTrackingCardProps) {
  ...
  async function handleDeleteFile(file: ScopeFile) {
    if (!window.confirm(`Excluir o escopo "${file.name}"? Esta ação não pode ser desfeita.`)) return
    try {
      const updated = await removeScopeFile(project.id, file.id)
      onProjectChange(updated)
      notify('Escopo removido.', 'info')
    } catch {
      notify('Não foi possível remover o escopo.', 'error')
    }
  }
    ...
    <Card className="p-5">
      ...
      {(project.scopeFiles ?? []).length > 0 && (
        <ul className="mt-3 space-y-1">
          {(project.scopeFiles ?? []).slice(0, 5).map((file) => (
            <li
              key={file.id}
            ...
            >
              {file.url ? (
                ...
                <span className="flex min-w-0 flex-1 items-center gap-2" title="Anexo antigo sem arquivo salvo — reenvie para poder baixar.">
                  <FileText className="size-3.5 shrink-0 text-slate-400" />
                  <span className="min-w-0 flex-1 truncate">{file.name}</span>
                  <span className="shrink-0 text-slate-400">{formatDate(file.uploadedAt)}</span>
                </span>
              ...
              <button
                ...
                className="inline-flex size-7 shrink-0 items-center justify-center rounded-md text-slate-500 transition-colors hover:bg-white hover:text-red-600 focus-visible:ring-2 focus-visible:ring-red-400 focus-visible:outline-none"
