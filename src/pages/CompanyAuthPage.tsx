...
import { registerCompany, resendCode, requestPasswordReset } from '@/services/companyAuthService'
...
export function CompanyAuthPage() {
  ...
  const onResend = () =>
    run(async () => {
      ...
      setDevCode(res.devCode)
      setInfo('Código reenviado para seu e-mail.')
    })
  ...
  const onForgot = (e: FormEvent) => {
    ...
    run(async () => {
      ...
      setDevCode(res.devCode)
      setInfo('Se o e-mail existir, enviamos um código.')
      setMode('reset')
    })
  }
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-7 shadow-xl">
          ...
          {mode === 'verify' && (
            <>
              ...
              <button type="button" onClick={onResend} disabled={busy} className={linkCls}>
          ...
          {(mode === 'login' || mode === 'register') && (
            <>
              ...
              <button type="button" onClick={() => switchMode(mode === 'login' ? 'register' : 'login')} className={linkCls}>
