...
export function ClientProjectDetailPage() {
  ...
  const orderedPhases = [...project.phases].filter(isVisible).sort((a, b) => a.order - b.order)
  const isClosed = project.status === 'encerrado'
  // Placar do cliente (premia ação: aprovações, tarefas, NPS).
  ...
  return (
    <>
      ...
      <div className="lg:grid lg:grid-cols-[minmax(0,1fr)_360px] lg:items-start lg:gap-6 xl:grid-cols-[minmax(0,1fr)_400px]">
        {/* ── Jornada: direita (sticky) no desktop, topo no mobile ── */}
        <div className="mb-5 lg:order-2 lg:mb-0 lg:sticky lg:top-6 lg:max-h-[calc(100vh-3rem)] lg:overflow-y-auto">
          <ClientGameHub project={project} />
        </div>

        {/* ── Conteúdo: esquerda, rolável ── */}
        <div className="space-y-5 lg:order-1">
          {/* Topo / cabeçalho do projeto */}
          <Card className="p-6">
          ...
          {isClosed && !project.nps && (
            <NpsGate projectId={project.id} hoursAfter={project.supportHours.depois} onAnswered={reload} />
          )}
          {isClosed && project.nps && <FinalizationUpsell project={project} />}
          {isClosed && <HoursBreakdown project={project} earnedPoints={game.phasePoints} />}

          {/* ── Precisa de você (seção destacada) ── */}
          <Card
            className={cn(
              'overflow-hidden',
              pending.length > 0 && 'border-amber-200 ring-1 ring-amber-100',
            )}
          >
            <div
              className={cn(
                'flex items-center justify-between gap-3 border-b p-5',
                pending.length > 0 ? 'border-amber-100 bg-amber-50/60' : 'border-slate-100',
              )}
            >
              <div>
                <h2 className="flex items-center gap-2 text-lg font-semibold text-slate-900">
                  <Clock className={cn('size-5', pending.length > 0 ? 'text-amber-500' : 'text-slate-400')} />
                  Precisa de você
                </h2>
                ...
                {clientTasks.map(({ phaseId, phaseName, item }) => (
                  <li key={item.id} className="p-5">
                    <div className="mb-2 flex items-start gap-2">
                      ...
                      <div className="min-w-0">
                        <div
                          className={cn('font-medium', item.done ? 'text-slate-400 line-through' : 'text-slate-800')}
                        >
                          {item.label}
                        </div>
                    ...
                    <div className="pl-7">
                      <CommentThread
                        ...
                        onAdd={(body, _mentions, attachments) =>
                          handleAddClientComment(phaseId, item.id, body, attachments)
                        }
                        side="cliente"
